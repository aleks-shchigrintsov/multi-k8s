...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=shchigrintsov/multi-server:$SHA
kubectl set image deployments/client-deployment client=shchigrintsov/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=shchigrintsov/multi-worker:$SHA
